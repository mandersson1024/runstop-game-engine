...
float Time()
{
    ...
    return 0.0000001f * static_cast<float>(time.QuadPart);
}
...
bool CGpuProfiler::Init(ID3D11Device* device, ID3D11DeviceContext* context)
{
	m_device = device;
	m_context = context;
	...
	if (FAILED(m_device->CreateQuery(&queryDesc, &m_apQueryTsDisjoint[0])))
	{
		ErrorPrintf("Could not create timestamp disjoint query for frame 0!");
		return false;
	}

	if (FAILED(m_device->CreateQuery(&queryDesc, &m_apQueryTsDisjoint[1])))
	{
	...
	{
		if (FAILED(m_device->CreateQuery(&queryDesc, &m_apQueryTs[gts][0])))
		{
			ErrorPrintf("Could not create start-frame timestamp query for GTS %d, frame 0!", gts);
			return false;
		}

		if (FAILED(m_device->CreateQuery(&queryDesc, &m_apQueryTs[gts][1])))
		{
...
{
	m_context->Begin(m_apQueryTsDisjoint[m_iFrameQuery]);
	Timestamp(GTS_BeginFrame);
}

void CGpuProfiler::Timestamp (GTS gts)
{
	m_context->End(m_apQueryTs[gts][m_iFrameQuery]);
}

void CGpuProfiler::EndFrame ()
{
	Timestamp(GTS_EndFrame);
	m_context->End(m_apQueryTsDisjoint[m_iFrameQuery]);
...
void CGpuProfiler::WaitForDataAndUpdate ()
{
	...
	// Wait for data
	while (m_context->GetData(m_apQueryTsDisjoint[m_iFrameCollect], NULL, 0, 0) == S_FALSE)
	{
		Sleep(1);
	}
	...
	if (m_context->GetData(m_apQueryTsDisjoint[iFrame], &timestampDisjoint, sizeof(timestampDisjoint), 0) != S_OK)
	{
	...
	if (m_context->GetData(m_apQueryTs[GTS_BeginFrame][iFrame], &timestampPrev, sizeof(UINT64), 0) != S_OK)
	{
	...
	{
		UINT64 timestamp;
		if (m_context->GetData(m_apQueryTs[gts][iFrame], &timestamp, sizeof(UINT64), 0) != S_OK)
		{
